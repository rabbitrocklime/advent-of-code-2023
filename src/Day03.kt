...
fun main() {
    println(Day03().puzzle1())
    println(Day03().puzzle2())
}
...
class Day03 {
    ...
    fun puzzle2(): Int {
        return starList.sumOf { star ->
            val nextToStar =
                numbers.filter { nums -> neighbours(nums).any { yx -> yx == star } } // filter out any that aren't adjacent to this star

            if (nextToStar.size == 2) {
                nextToStar[0].second * nextToStar[1].second
            } // if exactly 2 adjacent, return the gear ratio, else 0
            else 0
        }
    }
}